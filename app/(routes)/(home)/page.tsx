   ...
   async function getData(){
     ...
     if(selectedIds.length > 0){
       ...
       if(!requestData.some(data=>data.id === latestId)){
         try { 
           const response = await axios.get(`https://api.coingecko.com/api/v3/coins/${latestId}/market_chart?vs_currency=usd&days=1&x_cg_demo_api_key=CG-8CfAjVRc52vDuq5qtrTNDD1p`,{signal})
           setStatus('success')
    ...
    setStatus('loading')  
        try {
          selectedIds.map(async(item)=>{
            try {
              const response =  await axios.get(`https://api.coingecko.com/api/v3/coins/${item}/market_chart?vs_currency=usd&days=${days}&x_cg_demo_api_key=CG-8CfAjVRc52vDuq5qtrTNDD1p`)
